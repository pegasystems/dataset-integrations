...
public final class PgpEncryptionKeys {
    ...
    PgpEncryptionKeys() throws PGPException {
        this("password");
    }

    public PgpEncryptionKeys(String passphrase) throws PGPException {
        this.passphrase = passphrase;
        PGPKeyRingGenerator krgen = generateKeyRingGenerator("test@pega.com", passphrase.toCharArray());

        PGPPublicKeyRing pkr = krgen.generatePublicKeyRing();
        publicKey = generateKey(pkr);
        PGPSecretKeyRing skr = krgen.generateSecretKeyRing();
        privateKey = generateKey(skr);
    }


    private byte[] generateKey(PGPKeyRing pkr) {
    ...
    private static PGPKeyRingGenerator generateKeyRingGenerator(String id, char[] pass) throws PGPException {
        return generateKeyRingGenerator(id, pass, 0x02);
    }
    ...
    private static PGPKeyRingGenerator generateKeyRingGenerator(String id, char[] pass, int s2kcount) throws PGPException {
        // This object generates individual key-pairs.
        RSAKeyPairGenerator kpg = new RSAKeyPairGenerator();

        // Boilerplate RSA parameters, no need to change anything
        // except for the RSA key-size (2048). You can use whatever key-size makes sense for you -- 4096, etc.
        kpg.init(new RSAKeyGenerationParameters(BigInteger.valueOf(0x10001), new SecureRandom(), KEY_STRENGTH, CERTAINTY));
        ...
        PBESecretKeyEncryptor pske = (new BcPBESecretKeyEncryptorBuilder(PGPEncryptedData.AES_128, sha256Calc, s2kcount)).build(pass);

        // Finally, create the keyring itself. The constructor takes parameters that allow it to generate the self signature.
        PGPKeyRingGenerator keyRingGen =
                new PGPKeyRingGenerator(PGPSignature.POSITIVE_CERTIFICATION, rsakpSign,
